...
lemma degree_calculation {α : Type} [DecidableEq α] [Fintype α] (F : SetFamily α) (x : α) :
  degree F x = (Finset.filter (λ s => F.sets s ∧ x ∈ s) (F.ground.powerset)).card :=
  by
    simp_all only [degree, Finset.card_filter]
    simp [Finset.filter]
...
lemma erase_ground_card {α : Type} [DecidableEq α] [Fintype α] (F : SetFamily α) (v : α) (hv: v ∈ F.ground) (deg1: degree F v = 1) (hasGround: F.sets F.ground)(gcard: F.ground.card ≥ 2) :
  ...
  --have groundcard: ({F.ground} : Finset _).card = 1 := by
  --  simp
...
lemma total_degone_card {α : Type} [DecidableEq α] [Fintype α] (F : SetFamily α) (v : α) (hv: v ∈ F.ground) (deg1: degree F v = 1) (hasGround: F.sets F.ground)(gcard: F.ground.card ≥ 2) :
  ...
  --have h1 := erace_ground_card F v hv deg1 hasGround gcard
...
lemma powerset_bij {α : Type} [DecidableEq α][Fintype α]  (FG : Finset α) (a : α) (h : a ∈ FG) :
  ...
  apply @Finset.card_bij (Finset α) (Finset α) ss tt i
    ...
    (by
      ...
      simp_all only [mem_powerset, i, ss, tt]
      simp_all only [mem_filter, mem_powerset, ss]
      obtain ⟨left, _⟩ := hS
      intro x hx
    ...
    (by
      ...
      have goal: ∃ a, ∃ (ha : a ∈ ss), i a ha = T := by
        ...
        have tmp_goal2: a ∈ insert a T := by
          simp_all only [mem_insert_self]
        use mem_filter.mpr ⟨mem_powerset.mpr tmp_goal, tmp_goal2⟩
        --  error hT has type  T ∈ tt : Prop but is expected to have type insert a T ⊆ FG : Prop


        simp_all only [mem_insert_self, mem_erase, ne_eq, not_false_eq_true, true_and]
        simp_all only [erase_insert_eq_erase, not_false_eq_true, erase_eq_of_not_mem]
      exact goal
    )


-- 補題1: 任意の要素 a ∈ FG に対して、a を含む部分集合の数は 2^(n-1) である
lemma count_subsets_containing_a {α : Type} [DecidableEq α][Fintype α]
  ...
  -- 前提:
  -- FG は型 α の Finset で、card FG = n
  -- a は FG に属する要素である

  -- 1. FG に a を含む部分集合の集合を考える
  -- FG.powerset.filter (fun S => a ∈ S) は、FG の冪集合のうち a を含む部分集合のみを集めた Finset (Finset α) である

  -- 2. a を含む部分集合は、a を固定し、残りの要素から部分集合を選ぶことと一対一対応する
  -- つまり、FG.erase a の冪集合に対応する
  -- ここで、FG.erase a は FG から a を取り除いた Finset α である

  -- 3. この対応を形式的に示すために、フィルターされた冪集合と FG.erase a の冪集合が同じ要素数を持つことを示す

  -- 4. FG.erase a の冪集合のカードは 2^(n - 1) であることを示す

  -- まず、FG.erase a のカードが n - 1 であることを確認する
  have h_erase_card : (FG.erase a).card = n - 1 := by
    -- FG.erase a は FG から a を取り除いたものであり、a は FG に含まれているため
    -- card (FG.erase a) = card FG - 1 = n - 1
    rw [Finset.card_erase_of_mem ha]
    subst h_n
    simp_all only


  -- 次に、FG.powerset.filter (fun S => a ∈ S) の冪集合が FG.erase a の冪集合と同型であることを示す
  -- 具体的には、FG.powerset.filter (fun S => a ∈ S) ≃ (FG.erase a).powerset
  -- これは、各 S ∈ FG.powerset で a ∈ S について、S = T ∪ {a} となる T ∈ (FG.erase a).powerset に対応する

  -- ここでは、card_eq_powerset_erase を補助的な補題として定義し、それを用いる
  have h_powerset_eq : (FG.powerset.filter (fun S => a ∈ S)).card = (FG.erase a).powerset.card := by
    -- フィルターされた冪集合と FG.erase a の冪集合の間に双射が存在することを利用する
    -- 具体的には、s ↦ s.erase a が FG.powerset.filter (fun S => a ∈ S) から (FG.erase a).powerset への双射である
    -- しかし、Lean の Mathlib には直接的な双射の構築は必要ないため、cardinality の等式を直接示す
    -- a を含む部分集合の数は、FG.erase a の冪集合の数と等しい
    exact (powerset_bij FG a ha)

  -- 補助的な補題を定義する
  -- これは、a を含む FG の部分集合の冪集合のカードが FG.erase a の冪集合のカードに等しいことを示す
  -- この補題は Mathlib に存在しないため、ここで定義する
  -- 具体的には、a を含む部分集合は FG.erase a の冪集合と一対一対応するため、そのカードは同じになる
  have finset_card_powerset_filter_eq_powerset_erase : (FG.powerset.filter (fun S => a ∈ S)).card = (FG.erase a).powerset.card :=
    by
      -- 双射を明示的に構築しなくても、部分集合の構造からカードが等しいことがわかる
      -- 具体的には、a を含む部分集合は FG.erase a の部分集合に a を加えることで得られる
      -- したがって、数は等しい
      exact h_powerset_eq

  -- 5. FG.erase a の冪集合のカードが 2^(n -1) であることを示す
  have h_powerset_erase_card : (FG.erase a).powerset.card = 2 ^ (n - 1) := by
    -- FG.erase a のカードは n -1 であり、Finset.powerset_card_of_card
    -- これは、Finset.powerset (FG.erase a) のカードが 2 ^ (FG.erase a).card であることを示す
    rw [Finset.card_powerset]
    rw [h_erase_card]
    --exact pow_one 2 ^ (n - 1)

  -- 6. これらを組み合わせて、最終的な等式を導出する
  -- (FG.powerset.filter (fun S => a ∈ S)).card = (FG.erase a).powerset.card = 2 ^ (n -1)
  rw [finset_card_powerset_filter_eq_powerset_erase]
  rw [h_powerset_erase_card]

-- 補助的な補題の定義 tmp_ruleで使っているが、tmp ruleをつかってない。
lemma card_eq_sum_ones {α : Type} [DecidableEq α][Fintype α]  (s : Finset α) :
  Finset.card s = s.sum (λ a => 1) := by
  simp_all only [sum_const, smul_eq_mul, mul_one]

lemma card_eq_sum_ones2 {α : Type} [DecidableEq α][Fintype α] {s : Finset α}: ∑ a ∈ s, 1 = s.card := by
  simp_all only [sum_const, smul_eq_mul, mul_one]

--使ってない上に似たような補題がある。消す。
lemma sum_filter_eq_card {α : Type} [DecidableEq α][Fintype α]  (s : Finset α) (p : α → Prop) [DecidablePred p] :
  (s.filter p).sum (λ a => 1) = (s.filter p).card := by
  simp_all only [sum_const, smul_eq_mul, mul_one]

def fn (a : α) (s : Finset α): Nat := if a ∈ s then 1 else 0
-- 和の数える順番の補題。
...
by
  ...
  --#check (@Finset.sum_congr (Finset α) Nat FG.powerset _ Finset.card (λ s => s.sum (λ a => 1))) rfl (λ s hs=> card_eq_sum_ones s)
  --使ってない
  --let tmp_rule :=  (@Finset.sum_congr (Finset α) Nat FG.powerset _ Finset.card (λ s => s.sum (λ a => 1))) rfl (λ s hs=> card_eq_sum_ones s)

  have rewrite : ∑ a ∈ FG, (filter (fun S => a ∈ S) FG.powerset).card = ∑ a ∈ FG, ∑ x ∈ (filter (fun S => a ∈ S) FG.powerset), 1 := by
    simp_all only [sum_const, smul_eq_mul, mul_one, sum_boole, Nat.cast_id]
  rw [rewrite]

  have rewrite2 : ∑ a ∈ FG, ∑ x ∈ (filter (fun S => a ∈ S) FG.powerset), 1 = ∑ a ∈ FG, ∑ x ∈ FG.powerset, fn a x := by
    simp_all only [sum_const, smul_eq_mul, mul_one]
    have rewrite3:  ∀ a, ∑ x ∈ (filter (fun S => a ∈ S) FG.powerset), 1 = ∑ x ∈ FG.powerset, fn a x := by
       intro a
       simp [sum_congr rfl]
       dsimp [fn]
       simp
  ...
  have rewrite4: ∑ s ∈ FG.powerset, ∑ a ∈ s, 1 = ∑ s ∈ FG.powerset, ∑ a ∈ FG, fn a s := by
    ...
    rfl
    intro x a
    simp_all only [sum_const, smul_eq_mul, mul_one, mem_powerset, sum_ite_mem]
    congr
    exact (inter_eq_right.mpr a).symm
...
theorem powerset_sum_card_eq_card_mul_pow {α : Type} [DecidableEq α][Fintype α]
  ...
  have sum_eq_sum : FG.powerset.sum Finset.card = (FG.sum (fun a => (FG.powerset.filter (fun S => a ∈ S)).card)) := by
    ...
    have left_hand_side : FG.powerset.sum Finset.card = ∑ s ∈ FG.powerset, ∑ a ∈ s, 1:= by
      simp_all only [sum_const, smul_eq_mul, mul_one]
    have right_hand_side : FG.sum (fun a => (FG.powerset.filter (fun S => a ∈ S)).card) = ∑ a ∈ FG, (filter (fun S => a ∈ S) FG.powerset).card := by
      simp_all only [ge_iff_le, one_le_card, sum_const, smul_eq_mul, mul_one, n]
    ...
    simp [card_eq_sum_ones2] at scp
    --goal ∑ x ∈ FG.powerset, x.card = ∑ a ∈ FG, (filter (fun S => a ∈ S) FG.powerset).card
    convert sum_card_powerset_eq_sum_subsets_containing
    simp_all only [one_le_card, mem_powerset, sum_const, smul_eq_mul, mul_one, n]
    rename_i inst_1
    simp_all only [one_le_card, n]
    exact inst_1
    --強引に解決。
  ...
  have sum_contrib : FG.sum (fun a => (FG.powerset.filter (fun S => a ∈ S)).card) = FG.card * 2 ^ (n - 1) := by
    ...
    simp_all only [ge_iff_le, one_le_card, smul_eq_mul, n]
    ring
...
lemma ground_minus_v_ideal_total {α : Type} [DecidableEq α] [Fintype α] (F : IdealFamily α) (v : α) (hv: v ∈ F.ground)  (hv_hyperedge:F.sets (F.ground \ {v}))(hv_singleton:  ¬F.sets {v})(hcard0: F.ground.card >= 2):
  total_size_of_hyperedges F.toSetFamily = (F.ground.card - 1)*2^(F.ground.card - 2) + F.ground.card := by
        ...
        rw [formula]
        simp_all only [card_erase_of_mem, mul_eq_mul_left_iff, Nat.ofNat_pos, ne_eq, OfNat.ofNat_ne_one,
  ...
  degree F.toSetFamily v = 1 ↔ ¬F.sets {v} := by
    ...
    · intro h
      ...
      have lem_subset: (insert ({v}:Finset α) (singleton F.ground)) ⊆ F.ground.powerset.filter (λ (s:Finset α) => (F.sets s ∧ v ∈ s))  := by
        ...
        · intro x1
          intro a_1
          simp_all only [ge_iff_le, eq_iff_iff, iff_true, card_singleton, ne_eq, implies_true, and_self, and_true,
            true_and, mem_insert, mem_singleton]
          cases hs
          case inl h_1 =>
            subst h_1
        ...
        · simp_all only [ge_iff_le, eq_iff_iff, iff_true, card_singleton, ne_eq, implies_true, and_self, and_true,
          ...
          | inl h_1 =>
            subst h_1
            simp_all only [mem_singleton, and_self]
          | inr h_2 =>
            subst h_2
            simp_all only [and_self]
      ...
      have hcard2: ((insert ({v}:Finset α) (singleton F.ground)):Finset (Finset α)).card = 2 := by
        ...
        simp_all only [ge_iff_le, eq_iff_iff, iff_true, mem_singleton, ne_eq, mem_filter, mem_powerset,
          singleton_subset_iff, and_self, subset_refl, and_true, true_and, not_false_eq_true, card_insert_of_not_mem,
  ...
  (∀ (s:Finset α), s ∈ S → ¬(P s ∧ Q s)) →
   ...
   = ((Finset.filter (λ (s : Finset α) => P s) S).sum Finset.card) +
    ...
    have d_union:domain = rangeP ∪ rangeQ := by
      ...
      constructor
      intro a
      simp_all only [not_and, mem_filter, mem_union, true_and, domain, rangeP, rangeQ]
      intro a
      simp_all only [not_and, mem_union, mem_filter, rangeP, rangeQ, domain]
      cases a with
    ...
    have disjoint: rangeP ∩ rangeQ = ∅ := by
      ...
      intro h
      obtain ⟨hP, hQ⟩ := h
      simp_all only [not_and, mem_filter, and_false, rangeP, rangeQ]
...
--結果的にtotal degree cardと同じことを示している。
--lemma total_degone_card {α : Type} [DecidableEq α] [Fintype α] (F : SetFamily α) (v : α) (hv: v ∈ F.ground) (deg1: degree F v = 1) (hasGround: F.sets F.ground)(gcard: F.ground.card ≥ 2) :
--  total_size_of_hyperedges F = (F.ground.powerset.filter (λ s => F.sets s ∧ v ∉ s )).sum Finset.card + F.ground.card
  ...
  have disj: ∀ (s:Finset α), s ∈ F.ground.powerset → ¬((F.sets s ∧ v ∉ s) ∧ s = F.ground) := by
    ...
    intro h
    obtain ⟨h1, h2⟩ := h
    --let dohp := degree_one_hyperedges_partition2 F v hv s
    obtain ⟨left, right⟩ := h2
    exact h1.2 hv

  have ground_lem: F.ground.card = (filter (fun s => s = F.ground) F.ground.powerset).sum card := by
    --simp_all only [ge_iff_le, not_and, and_imp]
    rw [filter_eq']
    simp

    --simp_all only [mem_powerset, subset_refl, ↓reduceIte, sum_singleton]

  rw [ground_lem]

  have eqn0: (filter (fun s => F.sets s) F.ground.powerset) = (filter (fun s => (F.sets s ∧ v ∉ s) ∨ s = F.ground) F.ground.powerset) := by
    apply filter_congr
    intro s
    rw [Finset.mem_powerset]
    intro _
